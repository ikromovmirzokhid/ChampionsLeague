...
public class ClubMatchesAdapter1 extends RecyclerView.Adapter<ClubMatchesAdapter1.ViewHolder> {
    private List<MatchdayModel1> matchesList;
    private int win1, win2, draw1, draw2, loss1, loss2, point1, point2;

    public void setMatchesList(List<MatchdayModel1> matchesList) {
        this.matchesList = matchesList;
    }

    public ClubMatchesAdapter1(List<MatchdayModel1> matchesList) {
        this.matchesList = matchesList;
    }
        ...
        holder.firstTeamScore.setOnClickListener(v -> {
            ...
            menu.setOnMenuItemClickListener(item -> {
                switch (item.getItemId()) {
                    case R.id.zero:
                        holder.firstTeamScore.setText("" + 0);
                        break;
                    case R.id.one:
        ...
        holder.saveEditBtn.setOnClickListener(v -> {
            if (!holder.firstTeamScore.getText().toString().equals("-") && !holder.secondTeamScore.getText().toString().equals("-")) {
                ...
                if (holder.saveEditBtn.getText().toString().equals("save")) {
                    ...
                    holder.firstTeamScore.setClickable(false);
                    holder.secondTeamScore.setClickable(false);
                    setMatchesList(Database.getInstance().getMatchesDataList(matchesList.get(position).getMatchday()));
